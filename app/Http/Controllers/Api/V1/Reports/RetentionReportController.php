...
class RetentionReportController extends V1Controller
{
    public function __construct(private readonly ClinicReportService $reports) {}

    public function __invoke(Request $request): JsonResponse
    {
        $result = $this->reports->retention(
            $this->clinic($request),
            $request->query('period'),
        );

        return ApiResponse::success($result->toArray(), __('reports.retention_loaded'));
    }
}
